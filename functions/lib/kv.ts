...
// Daily *credit* counter. Replaces the previous request-count counter; the
// `:c:` prefix keeps old `rl:d:*` buckets from interfering during rollout.
export function dailyCreditsKey(scope: string, epochSeconds?: number): string {
  return `rl:c:${scope}:${utcDayStamp(epochSeconds)}`;
}
...
export async function incrementCounter(
  ...
  const next = (raw ? parseInt(raw, 10) : 0) + delta;
...
export async function getDailyCredits(env: Env, scope: string): Promise<number> {
  return readCounter(env.RATE_LIMIT_KV, dailyCreditsKey(scope));
}

export async function bumpMinuteCounter(env: Env, scope: string): Promise<number> {
  return incrementCounter(env.RATE_LIMIT_KV, minuteKey(scope), MINUTE_TTL, 1);
}

export async function bumpDailyCredits(
  env: Env,
  scope: string,
  cost: number
): Promise<number> {
  return incrementCounter(env.RATE_LIMIT_KV, dailyCreditsKey(scope), DAILY_TTL, cost);
}
